...
def query_qna(text, filename, current_service_context, verbose=False):
    ...
    QA_PROMPT_TMPL = """You are a large language model whose expertise is reading and summarizing documents. 
    ...
    index = GPTSimpleVectorIndex.load_from_disk(
        f"./data/{filename}_vector.json", service_context=current_service_context
    )
    res = index.query(
        text, verbose=True, similarity_top_k=3, text_qa_template=QA_PROMPT
    )
    return res
...
def query_highlight(text, filename, current_service_context, verbose=False):
    ...
    QA_PROMPT_TMPL = """You are a large language model whose expertise is reading and summarizing scientific papers. 
